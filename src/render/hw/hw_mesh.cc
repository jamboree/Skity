...
uint32_t HWMesh::AppendVertex(float x, float y, float mix, float u, float v) {
  uint32_t base = VertexBase();
...
uint32_t HWMesh::AppendVertex(const HWVertex &vertex) {
  uint32_t base = VertexBase();
...
uint32_t HWMesh::AppendIndices(const std::vector<uint32_t> &indices) {
  uint32_t base = IndexBase();
